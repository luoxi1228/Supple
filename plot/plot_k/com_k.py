...
def read_data(file_path: Path):
    """Read x from column 4, online from -3rd, offline from -4th."""
    data = {}
...
def main():
    ...
    # ===== Inset: zoom in on 4, 16, 64 (plot_b-style) =====
    zoom_idx = [0, 1, 2]
    zoom_x = [x_pos[i] for i in zoom_idx]
    axins = ax.inset_axes([0.15, 0.42, 0.52, 0.48])
    ...
    axins.set_ylim(-1.2, 2.5)
    axins.set_yticks([-1, 0, 1, 2])
    ...
    fig.savefig(base / "com_k.pdf", bbox_inches="tight", pad_inches=0.01)
    fig.savefig(base / "com_k.png", dpi=600, bbox_inches="tight", pad_inches=0.01)
