...
def main():
    ...
    # 约等于最终论文里半宽子图尺寸
    PANEL_W = 2.40   # inch ≈ 6.0 cm
    PANEL_H = 1.80   # inch ≈ 4.5 cm
    ...
    inset.set_ylim(-0.4, 5)
    inset.set_yticks([i * 2 for i in range(3)])
    ...
    fig.savefig(base / "plot_n.pdf", bbox_inches="tight", pad_inches=0.01)
    fig.savefig(base / "plot_n.png", dpi=600, bbox_inches="tight", pad_inches=0.01)
    # fig.subplots_adjust(left=0.19, right=0.98, bottom=0.18, top=0.99)

    # fig.savefig(base / "plot_n.pdf")
    # fig.savefig(base / "plot_n.png", dpi=600)
